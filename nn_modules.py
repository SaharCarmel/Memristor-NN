...
class manhattan_net(torch.nn.Module):
    def __init__(self, args):
        ...
        self.fc1_pos = Memristor_layer(
            784, 800, lower=args.lower, upper=args.upper)
        self.fc1_neg = Memristor_layer(
            784, 800, lower=args.lower, upper=args.upper)
        self.fc2_pos = Memristor_layer(
            800, 10, lower=args.lower, upper=args.upper)
        self.fc2_neg = Memristor_layer(
            800, 10, lower=args.lower, upper=args.upper)
        self.bias_vector = torch.zeros([28,28])+args.bias_voltage
        self.bias_output = 0
        self.criterion = nn.CrossEntropyLoss(reduction='sum')
        self.lr = args.lr
    ...
    def forward(self, x):
        ...
        x = x - self.bias_output
        return x

    def compute_bias(self):
        x = self.bias_vector.view(-1, 28*28)
        x = F.relu(self.fc1_pos(x)-self.fc1_neg(x))
        x = self.fc2_pos(x) - self.fc2_neg(x)
        self.bias_output = x
